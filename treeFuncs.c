...
struct tree *alctree(int label, short nkids, ...) {
    ...
    if (nkids > 0 && nkids <= 9) {
        ...
        va_start(ap, nkids);            //free(ptr->u.leaf);
        for (i = 0; i < nkids; i++) {
            t->u.child[i] = va_arg(ap, struct tree*);
        }
        va_end(ap);
        t->u.child[nkids] = calloc(9-nkids, sizeof(struct tree*));
    } else if (nkids == -1) { //Special case for terminals to set their leaf token

        nkids = 1;
        va_start(ap, nkids);
        t->u.leaf = va_arg(ap, struct token*);
        va_end(ap);
        //for(i = 1; i < 9; i++) t->u.child[i] = NULL;
    }
    return t;
}
...
int printTree(struct tree *t, int depth) {
    ...
    if(t==NULL) {
        ...
        if((t->u.leaf != NULL) && (t->u.leaf->text != NULL)) printf("%*s leaf: %s\n", depth*2, " ", t->u.leaf->text);
